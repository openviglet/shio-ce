...
Create the site, shape the content model, write the pages, upload the media, publish or schedule it, and add the rest of the team. All in a browser, with no agent, no terminal and no file to hand-edit anywhere in it.
...
Pull the whole site to disk as front-mattered Markdown, edit it with the editor you already use, push it back through a three-way merge. Put shio apply --check in CI so drift fails a build instead of surprising somebody.
...
- P1 Agent-first design order Every capability lands MCP → files/CLI → REST → console. A console-only feature is incomplete. The order is about what may not be skipped on the way to shipping, never about who is allowed to operate the CMS.
- P2 One call replaces a session Discovery is a product feature, not a documentation problem: a manifest and a context pack.
...
P10 Hand-rolled rather than pulled from a vendor SDK, and the reason is written down with a date on it, and the premise is re-checked on a schedule and a test fails the build when it expires. [modelcontextprotocol.io](https://modelcontextprotocol.io)
...
A write lands as a draft. Publishing is a separate, explicit act with its own op. No flag quietly does both.
