...
Four verbs turn a site that exists into one a curator can edit. Not a mirror and not a screenshot, but a content model derived from the pages themselves, with a review step you cannot skip and arithmetic at the end that says whether anything was lost.
...
Measured, and printed on every run: four pages of four blocks costs about 294 tokens and sixty pages costs about 295. What grows is a path per page, not a candidate per page, and the per-block implementation an author naturally writes would have produced 240 candidates for the same site. P3
...
It is the complement of what the fields consumed, and the only definition that cannot *under*-report, and under-reporting is the one failure this artefact must not have.
