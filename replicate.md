...
Four commands, and each one makes a promise the next depends on. They are separate on purpose: a single button that crawled a stranger's site and wrote content into yours would be the surprise the whole design forbids.
...
The site's own theme. The source's styling is replaced, deliberately — not ruined.
