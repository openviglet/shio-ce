...
The site is the caller's, so a site key inside a package is refused at load rather than ignored, because an ignored key looks like it worked. And prune's blast radius depends on what an author forgot to mention, which is an unacceptable thing to inherit from a package the user did not write.
