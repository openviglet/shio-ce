...
| Path addressing, with no UUID carried between calls | yes | partial | partial | partial | partial |
...
This reflects 2026.3, so verify against current vendor documentation before deciding. If a row here is wrong, it is a bug and we want the correction.
